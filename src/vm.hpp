    ...
    void cycle() {
      ...
      */
      ...
      switch (this->extract(instruction, OP)) {
        ...
        case 0x08: {
          // Performs signed addition, on underflow or overflow will throw exception
          int rs = this->registers.at(this->extract(instruction, RS));
          short immd = this->extract(instruction, IMMD);

          if ((immd >= 0) && rs > std::numeric_limits<int>::max() - immd) {
            throw std::runtime_error("Integer overflow detected while performing an addi instruction!");
          } else if ((immd < 0) && rs < std::numeric_limits<int>::min() - immd) {
            throw std::runtime_error("Integer underflow detected while performing an addi instruction!");
          };

          std::cout << "ADDI -> " << std::dec << rs + immd << std::endl;
        ...
        case 0x06: {
          ...
          int rs = static_cast<int>(this->registers.at(this->extract(instruction, RS)));

          if (rs <= 0) {
            this->ip += 4 + (static_cast<short>(this->extract(instruction, IMMD)) << 2);
        ...
        case 0x0: {
          switch (this->extract(instruction, FN)) {
            ...
            case 0x2A: {
              int rs = static_cast<int>(this->registers.at(this->extract(instruction, RS)));
              int rt = static_cast<int>(this->registers.at(this->extract(instruction, RT)));
            ...
            case 0x20: {
              ...
              if ((rs >= 0) && rt > std::numeric_limits<int>::max() - rs) {
                throw std::runtime_error("Integer overflow detected while performing an add instruction!");
              } else if ((rs < 0) && rt < std::numeric_limits<int>::min() - rs) {
                throw std::runtime_error("Integer underflow detected while performing an add instruction!");
              };

              std::cout << "RS: " << std::dec << rs << std::endl;
                          std::cout << "RT: " << std::dec << rt << std::endl;

              std::cout << "ADD -> " << std::dec << rs + rt << std::endl;
