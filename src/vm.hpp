...
// NOTE THIS SHOULD BE A FORKED REPOSITORY AND DIRECTLY MODIFIED, DIFFERENT
// CHIPS HANDLE CERTAIN THINGS DIFFERNELTY INTERNALLY APART FROM IMPLEMENTING
// THE BASE INSTRUCTION SET. SOME CHIPS MAY HANDLE PIPELINING DIFFERENTLY, OR
// MAY HAVE CO PROCESSORS THAT INTERACT SEAMELESSLY WITH THE ACTUAL BASE CORE.
  ...
  void cycle() {
    ...
    */
    ...
    case 0x08: {
      ...
      if ((immd >= 0) && rs > std::numeric_limits<int>::max() - immd) {
        throw std::runtime_error(
            "Integer overflow detected while performing an addi instruction!");
      } else if ((immd < 0) && rs < std::numeric_limits<int>::min() - immd) {
        throw std::runtime_error(
            "Integer underflow detected while performing an addi instruction!");
      };
    ...
    case 0x0: {
      ...
      case 0x20: {
        // Performs signed addition, on underflow or overflow will throw
        // exception
        int rs = this->registers.at(this->extract(instruction, RS));
        int rt = this->registers.at(this->extract(instruction, RT));
